...
| 3   | Three text-safe variants          | **`--clay-ink #e0a488` (dark), `#8f5138` (light); light `--brass #8a6320`; light `--ink-faint #5f6b5a`** | Raw cedar is 3.97:1 on basalt and 3.29:1 on mist; raw golden is 1.88:1 on mist. Fills keep the bright pigment, text gets its own value. One exception is measured and named: mist on raw cedar = 4.55:1, so initials avatars may use the raw fill.                                                                                                                                                     |
