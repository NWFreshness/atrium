...
npx playwright test  # needs AUTH_OWNER_EMAIL/PASSWORD
```
...
CI: the default `ci` job stays unit/build only and does not need secrets. A separate `e2e` job installs Playwright Chromium and runs `npx playwright test` only when `DATABASE_URL`, `AUTH_SECRET`, `AUTH_OWNER_EMAIL`, and `AUTH_OWNER_PASSWORD` GitHub secrets exist; otherwise it skips successfully.
