...
### 7.2 Atomic demo reset (completed)

Reset demo is one Neon transaction. New `lib/db/batch-transaction.ts`: a `WriteBatch` collects statements instead of running them, `createBatchRunner(getDb)` is now `resetDemo`'s default (production) `runInTransaction`, and `isWriteBatch` / `WriteOpts` let the resetters and seeds branch on being handed one. Every `createX` helper in `lib/{crm,space,rolodex}/queries.ts` takes a trailing `opts?: WriteOpts` and, when it is set, collects the insert instead of awaiting one — after the memory-repo branch, before the Drizzle insert, returning the app-built row so the seed can keep using its ids. Space's seed also stopped reading the database for positions: it assigns them from a per-parent counter and hands the tree's page ids back, which is the only way a batched reseed can know what it is about to insert (the rows are not visible to a query while statements are still being collected). CRM's wipe is now four tenant-scoped deletes (`activities → deals → contacts → organizations`) instead of `listActivities` → `deleteActivity` per row, matching Space and Rolodex.

The whole reset is **one batch of 717 statements** — 18 deletes, 699 inserts (3 orgs, 4 contacts, 6 deals, 4 activities, 32 people and their children, 51 pages, 499 blocks, 11 properties, 14 options, 61 row values, 3 views), ~89 KB of SQL and 4,767 bind parameters — measured with the three real module-load resetters against a recording `Database` over an unconnected one.

Verified live against the Neon **dev** branch as the demo user, not only in unit tests: one Reset demo click → one `POST` on `app/(authenticated)/layout.tsx`, ~1.1 s of application time, no server error, no console error; the Space tree went 44 → 38 sidebar links (the accumulated e2e rows are gone) with a new Home page id, CRM listed 3 organizations and Rolodex 32 people afterwards, and a second reset produced identical counts. That answers the one question unit tests could not: Neon accepts a 717-statement batch as a single transaction, and all three apps participate in it.
...
### 7.4 Split query modules (completed)

Each app's query module is four files now: `queries.ts` (the public barrel — `requireTenantId`, row construction, dispatch, and the `createMemory<App>Repository` re-export), `queries-shared.ts` (types plus the pure helpers both stores need), `queries-memory.ts` (the repo factory and every `*InMemory` impl) and `queries-drizzle.ts` (the only file per app importing `../db`, holding all the SQL). CRM's barrel went 942 → 399 lines, Space's 1281 → 615, Rolodex's 1015 → 481, with the halves at 353/436, 576/583 and 356/462. No action, seed, reset or test file needed an edit, and `package.json` is untouched.

A third module per app is beyond the spec's pair: both halves need `requireTenantId`/`clone`/`now`/the search predicates, so without `queries-shared.ts` they would import each other or the barrel — a runtime cycle. Space's `insertBlockAt`/`reorderBlocks` stayed in the barrel because they compose public dispatchers and production calls them with `repo` undefined. The barrels still build the rows (both stores receive the identical row — that is what keeps 7.2's batch path working); what left them is every Drizzle implementation.

Verified as an equivalence rather than "the tests still pass": each barrel's runtime export list (21/33/27 names) was captured before the split and is unchanged after; per-app token counts equal HEAD's for every SQL operator plus `clone(`, `now()` and `newId()`; the `new Error` message sets are identical; every create/update memory impl still returns `clone(...)` (no live-array aliasing); timestamps are built only in the barrels, so no row gets two. Controller: `env -u DATABASE_URL npm test` 633 passed (99 files — the same count as before), `npm run build` exit 0, `git diff -- package.json package-lock.json` empty.

Two review subagents (spec compliance plus an adversarial semantics pass) found no blocking issue and produced five fixes, all applied: Space's Drizzle module imported 17 types without the `type` keyword (fragile under `isolatedModules`); Rolodex's Drizzle create impls were named `create<X>InDrizzle` where CRM and Space use `insert<X>` (renamed, since 7.5 will grep these modules); Space's Drizzle position helpers lacked the `InDrizzle` suffix their memory twins carry; twelve create-in-memory impls carried an unused `scoped` parameter; and `lastContactedFor`/`latestNewsFor` were exported while only `enrichPerson` uses them.

Three differences remain and are recorded as equivalent: six Rolodex list functions now guard `tenantId` in the barrel instead of one level down inside `listChildren` (+5 calls, same message — verified by calling all eight with a blank tenant), CRM's `nextBoardOrderInMemory` is synchronous where HEAD's helper was `async` (one microtask of timing, unobservable), and the three splits differ cosmetically in dispatch style. The client leak stays for 7.5, and the split adds no new module edge to that chunk: HEAD's `queries.ts` already imported `../db` and already held every in-memory branch.

### 7.5 Client/server import boundary (completed)

`lib/client-boundary.test.ts` is the fitness function. It strips comments with a scanner that understands string and regex literals, reads every module reference — static `import`/`export … from`, side-effect, and dynamic `import()`/`require()` — with no semicolon required anywhere, classifies each as erased or value, then walks the value-import graph from all 81 files under `components/`. 139 files are visited because the walk crosses into `lib/`; it stops at `"use server"` modules and fails on `lib/db`, any `lib/{crm,space,rolodex}/{queries,schema,seed,reset}`, `drizzle-orm` or `@neondatabase/serverless`, matched by specifier _and_ by resolved path. Text that merely looks like an import (the same statement quoted inside a template literal) is rejected by span position, and a dynamic import whose target is not a literal fails as unreadable rather than being assumed harmless.

The leak it was written for is gone: `circleColumnStats` moved out of `move-person.ts` — which value-imports `queries.ts` — into `lib/rolodex/circle-stats.ts`, and the board imports it from there. `move-person.ts` keeps `movePersonCircle`; the helper's test block moved to `lib/rolodex/circle-stats.test.ts`.

Verified as an equivalence, not just a green suite. The gate was **red on main**, reporting exactly `lib/rolodex/move-person.ts → ./queries`, and then thirteen injection probes were applied and reversed byte-identically: the transitive chain, a direct barrel import, a side-effect import, a dynamic import, `export * from`, `lib/db`, a semicolon-free file, a second statement after a `;` on the same line, the string-blind comment trick, an unreadable dynamic import, and a regex literal containing quotes all fail; `import type` and `import { type X }` from the barrel stay green. Controller: `env -u DATABASE_URL npm test` **639 passed (101 files)**, up from 633 (99); build exit 0; `git diff -- package.json package-lock.json` empty; prettier per explicit path.
...
### 9.2 Isolation headers (completed)
...
### 9.6 Text ceilings + LIKE escape (completed) — Phase 9 closed

Stored free text has an application ceiling; CRM ILIKE treats percent, underscore, and backslash as literals. `lib/input/text.ts` owns `MAX_SHORT_TEXT` (500), `MAX_LONG_TEXT` (8_000), `MAX_BLOCK_TEXT` (20_000), `assertText`, and `escapeIlike`. The three query barrels call `assertText` on create/update string fields before memory/Drizzle dispatch. CRM Drizzle search binds `escapeIlike` into an ILIKE fragment with ESCAPE. Memory search stays `includes` on the raw term.

Verified: `env -u DATABASE_URL npm test` 692 passed (106 files); `AUTH_SECRET=ci-build-placeholder npm run build` exit 0. No Playwright. No new dependencies, no migration. Phase 9 is complete: 9.1–9.6. Nothing is queued.

### Vercel Web Analytics mount (completed)

Root-level infrastructure, not a phase feature: one dependency and one mount, requested directly. `@vercel/analytics` is pinned exact at `2.0.1` as a runtime dependency, and `<Analytics />` is mounted in `app/layout.tsx`'s `<body>` — the only import site under `app/`, `components/`, or `lib/`, which is what keeps a second client boundary (and a second pageview) from appearing later. `app/analytics.test.ts` (4 tests) is the gate: runtime-not-dev dependency, one import site, mount inside `<body>` and not `<head>`, no mount anywhere else. Written red first — 3 of the 4 failed before the layout edit.

Verified in a real Chromium, not just by asserting the tag exists. Dev (`next dev`): the injected script is `https://va.vercel-scripts.com/v1/script.debug.js` with `sdkn=@vercel/analytics/next`, `sdkv=2.0.1`; the request returned 200, the script executed, and it logged the queued `pageview {route: /login, path: /login}` — with zero console errors, zero warnings, zero page errors, no failed requests. Production (`next start`, insights endpoints stubbed because no Vercel edge exists locally): `/_vercel/insights/script.js` requested exactly once with `window.vam === "production"` and the stub script running. Controller: `env -u DATABASE_URL npm test` 696 passed (107 files, up from 692/106) and `AUTH_SECRET=ci-build-placeholder npm run build` exit 0 with the route table unchanged; eslint clean on both files; prettier applied per path.

Deliberate: no phase spec and no INDEX row (this is not a phase feature), the CSP still has no `script-src` (9.2's decision) so the external debug script is allowed in dev — adding `script-src` later needs `va.vercel-scripts.com` and `/_vercel/insights` in the allowlist — and no `NEXT_PUBLIC_VERCEL_*` var is set, because Vercel injects the observability ones itself. Not verified: an authenticated app screen; `.env` has no `AUTH_OWNER_*`/`AUTH_DEMO_*` pair, so no e2e login was possible. The mount is structural regardless — `app/(authenticated)/layout.tsx` renders a fragment inside the root `<body>`.

### Phase 10 PNW specs (written, not implemented)

Docs only — no application code in this commit. Two design studies came first: the six colours Tyler named (moss `#2d4a3e`, mist `#eef1f2`, charcoal `#25282a`, lichen `#a3b19b`, cedar `#b87355`, golden `#dfa84a`) measured pair-by-pair against each other, then ten typefaces measured from their loaded outlines. Both are in `docs/prototypes/2026-09-14-pnw/` as self-contained HTML with their own theme/pairing controls; nothing under `app/`, `components/`, or `lib/` imports them.

What the measurements settled: the six are a palette, not a theme. Moss is 1.53:1 on basalt — it is a **surface** on dark and the primary **ink** on light (8.56:1). Raw cedar (3.97 dark / 3.29 light) and raw golden (1.88 on mist) cannot carry text, so light mode derives its own text values and fills keep the bright pigment. The dark-mode faint-ink tier had to become solid lichen rather than a dimmed `rgba()`, because 62 %-opacity lichen measures 2.96:1 on a panel and WCAG has no micro-label exemption. Four chart series separate by hue, not luminance (1.0–1.29 apart), which is why direct labels are a spec requirement rather than a nice-to-have. On type: Outfit (49 % x-height, 522/1000em) over Archivo (53 %, 497/1000em — 7 % narrower than Inter, which a four-column table notices), with Lora/DM Sans recorded as the heritage alternative and a named trigger to flip.
