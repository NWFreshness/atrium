...
**None in progress.** Phase 7 Architecture: 7.1 (ADRs), 7.2 (atomic demo reset), 7.3 (`tenantId` indexes) and 7.4 (query-module split) are done. Next implementable unit is **7.5**, the client/server import boundary — and it is the unit that closes the phase. Branch `feat/7.5-…` from current `main`.
