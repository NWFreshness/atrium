...
**Stop. Nothing is in progress.** 9.4 is complete on this branch (awaiting merge). Do not start 9.5 until 9.4 has a `mergedAt`. Do not push to `main`. Do not merge unless asked.

## Where we are (2026-09-11)

Phases 0–8 and 9.1–9.3 are complete. 9.4 throttles repeated login and signup failures (`auth_throttles`). Next is 9.5 after this PR merges. After merge, `npm run db:migrate` on the Neon **dev** URL. Design: `docs/superpowers/specs/2026-09-11-security-design.md`.
...
6. After 9.4 merges: `features/phase-9-security/9.5-bounded-import.md`. Do not start 9.5 on this branch.
...
Phase 8 is complete and the dev branch carries every migration (`0000`–`0006`). 9.4 is complete on this branch (adds `0007`). After it merges: `npm run db:migrate` on the Neon **dev** URL, then `git checkout main && git pull`, branch `feat/9.5-bounded-import`, and implement 9.5 only.
